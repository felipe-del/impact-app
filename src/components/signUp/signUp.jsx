...
const SignUp = ({ switchToSignIn }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name || !email || !password) {
         ...
         /** Validate that the email ends with @ucr.ac.cr */
        if (!email.endsWith('@ucr.ac.cr')) {
            setRegisterError('Se debe de registrar con el correo institucional');
            return;
        }
          /** Validate password (minimum 8 characters, 2 numbers, 1 special character) */
        ...
        try{
            ...
            console.log(data);
            /** If registration is successful, clear the fields and set success message */
            if(data){
            ...
            setRegisterError('Registro fallido. Por favor revisa tus crendenciales y prueba de nuevo.');
        }
    };
     
    /** When input value changes, clear success and error messages */
     const handleInputChange = (setter) => (e) => {
        setter(e.target.value);
        setRegisterSuccess(''); /** Clear success message when typing */
        setRegisterError('');   /** Clear error message (optional) */
        setError('')            /** Clear general error state (if applicable) */
    }; 
